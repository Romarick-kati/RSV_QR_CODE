...
// Same polling approach and interval as the admin NotificationBell — see
// that file's comment for why (no persistent connection to keep open on a
// Netlify Function backend anyway).
const POLL_MS = 30000;

export default function AttendeeNotificationBell() {
  const { t } = useLanguage();
  ...
  useEffect(() => {
    ...
    function load() {
      notificationsApi.mine()
        ...
        .catch(() => {}); // silent — a failed poll shouldn't interrupt browsing
    }
    load();
          ...
          >
            ...
            <div className="max-h-[60vh] sm:max-h-96 overflow-y-auto">
              {notifications.length === 0 ? (
                <p className="text-sm text-[var(--text-dim)] text-center py-8 px-4">{t('notif_empty_attendee')}</p>
              ) : (
                notifications.map((n) => {
                  ...
                  return (
                    <Link
                      key={n.id}
                      to={n.meta?.eventId ? `/events/${n.meta.eventId}` : '/events'}
